...
def resize_pad(img,w_new=128,h_new=128):
    ...
    img2 = cv2.resize(img1, (w_new,h_new))
...
class BlazeLandmark(BlazeBase):
    ...
    def calc_eye_bbox(self, landmarks):
        ...
        left_eyes_lm, right_eyes_lm = self.get_eye_landmarks(landmarks)

        left_eye_bbox = self._calc_bounding_rect(left_eyes_lm)
        right_eye_bbox = self._calc_bounding_rect(right_eyes_lm)
...
def bbox_from_landmarks(points,landmarks):
